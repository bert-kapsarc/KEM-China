...
COprodcuts(r)
COprodcutsSOE
COsulfDW(sulf) sulfur content by dry weigh tfor each sulfur-content category

;

$gdxin db\coalprod.gdx
$load COprodyield COwashratio COprodData coalcv COomcst COsulfur COprodcuts
$load COcapacity COsulfDW
$gdxin
...
scalar    COdiscrate  fuel sector discounting rate;

          COdiscrate = discount_rate("CO");
...
*        metallurgical and hard coking coal are always washed
         COprodyield(coal,mm,ss,"raw",time,rco)$(COprodData(coal,mm,ss,time,rco)>0) = 1;
...
COprodData(met,mm,ss,time,rco) = COprodData(met,mm,ss,time,rco)*1.01;

*COprodData(coal_i,mm,ss,time,rco)$(Local(ss) or Allss(ss)) = COprodData("coal",mm,ss,time,rco)*0.05;
*COprodData(coal_i,mm,ss,time,rco)$(TVE(ss)) = COprodData("coal",mm,ss,time,rco)*0.28;
COprodData(coal_i,mm,ss,time,rco) = COprodData("coal",mm,ss,time,rco)*0.1;
COprodData("coal",mm,ss,time,rco) = COprodData("coal",mm,ss,time,rco)-COprodData("coal_i",mm,ss,time,rco);
...
*        Sets used to eliminate unecessary variable indexes from the model
         set COmine(COf,mm,ss,rco) coal mine units used in the model equations
             ...
             COmet2thermal(COf,COff) Transfer of met coal for coal consumption;
...
parameter  COomcst_tmp;

         COomcst_tmp("coal","under",ss2,rw,time,rco)$(rwashed(rw) and COomcst("coal","under",ss2,rw,time,rco)<=0)=1e6;

         COomcst_tmp("coal","under",ss2,rw,time,rco)$(rwother(rw) and coalcv('coal',"under",ss2,rw,time,rco)>0)=
         COomcst("coal","under",ss2,rw,time,rco)/coalcv('coal',"under",ss2,rw,time,rco);
         COomcst_tmp("coal","under",ss2,rw,time,rco)$(rwother(rw) and COomcst("coal","under",ss2,rw,time,rco)<=0)=1e6
         ;
...
*        estimate 60% additional cost for metallurgical coal production
         ...
         1.6*smin(ss2,COomcst_tmp("coal","under",ss2,rw,time,rco))$rwashed(rw)
         +1*smin(ss2,COomcst_tmp("coal","under",ss2,rw,time,rco))*coalcv(met,"under",ss,rw,time,rco)$rwother(rw);
...
*        The equation COsupplylim sets the upper bound of each supply step.
         ...
         COcapcuts(trun,r) capacity cuts applied regional suppliers
...
COcapcuts(t,r)..
sum((COf,mm,ss,rco)$(COmine(COf,mm,ss,rco) and not coal_i(COf) and rco_sup(rco,r)),
         COexistcp(COf,mm,ss,t,rco)
         +CObld(COf,mm,ss,t-COleadtime(COf,mm,rco),rco)
